...
function argumentos(argv = process.argv.slice(2)) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (['aplicar','retomar','permitir-projeto-diferente','regras-conferidas'].includes(k) && argv[i+1] && !argv[i+1].startsWith('--')) throw new Error('A opção --'+k+' não recebe valor. Omita a opção para não executar a ação.');
